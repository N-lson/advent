...
let rec oxygen depth (arr:string[]) = 
    ...
    if filtered.Length = 1 || depth = maxDepth - 1 then Array.head filtered else oxygen (depth + 1) filtered

let rec co2 depth (arr:string[]) = 
    let bit = if arr.[(arr.Length)/2].[depth] = '0' then '1' else '0'
    let filtered = Array.filter (fun (x:string) -> x.[depth] = bit) arr
    if filtered.Length = 1 || depth = maxDepth - 1 then Array.head filtered else co2 (depth + 1) filtered
